...
```
flow/initialize/SAMSUNG/S8/common/cmds/tool_kits/
├── steps/              # 主脚本目录
...
tool_kits:
  dependency:
    FP_MODE:
      - step1:
          sub_steps:
            tool_kits_sub_step1.tcl: tool_kits::sub_step1
            tool_kits_sub_step2.tcl: tool_kits::sub_step2
```

**格式说明**：
- Key：文件名（如 `tool_kits_sub_step1.tcl`），可以是任意文件名
- Value：proc 名称（如 `tool_kits::sub_step1`，使用 Tcl namespace 语法）
...
```tcl
# tool_kits_sub_step1.tcl
proc ::tool_kits::sub_step1 {} {
    global edp project tool_kits
