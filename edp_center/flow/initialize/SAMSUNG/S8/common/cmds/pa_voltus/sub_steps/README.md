...
```
flow/initialize/SAMSUNG/S8/common/cmds/pa_voltus/
├── steps/              # 主脚本目录
...
pa_voltus:
  dependency:
    FP_MODE:
      - step1:
          sub_steps:
            pa_voltus_sub_step1.tcl: pa_voltus::sub_step1
            pa_voltus_sub_step2.tcl: pa_voltus::sub_step2
```

**格式说明**：
- Key：文件名（如 `pa_voltus_sub_step1.tcl`），可以是任意文件名
- Value：proc 名称（如 `pa_voltus::sub_step1`，使用 Tcl namespace 语法）
...
```tcl
# pa_voltus_sub_step1.tcl
proc ::pa_voltus::sub_step1 {} {
    global edp project pa_voltus
