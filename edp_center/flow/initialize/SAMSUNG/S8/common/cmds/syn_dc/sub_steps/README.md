...
```
flow/initialize/SAMSUNG/S8/common/cmds/syn_dc/
├── steps/              # 主脚本目录
...
syn_dc:
  dependency:
    FP_MODE:
      - step1:
          sub_steps:
            syn_dc_sub_step1.tcl: syn_dc::sub_step1
            syn_dc_sub_step2.tcl: syn_dc::sub_step2
```

**格式说明**：
- Key：文件名（如 `syn_dc_sub_step1.tcl`），可以是任意文件名
- Value：proc 名称（如 `syn_dc::sub_step1`，使用 Tcl namespace 语法）
...
```tcl
# syn_dc_sub_step1.tcl
proc ::syn_dc::sub_step1 {} {
    global edp project syn_dc
