...
class TestCLIIntegration(unittest.TestCase):
    ...
    @patch('main.cli.commands.handlers.handle_load_config')
    def test_load_config_command(self, mock_handle):
        """测试 load-config 命令"""
        mock_handle.return_value = 0
        ...
        from main.cli.commands.handlers import handle_load_config
        result = handle_load_config(manager, args)
        
        # 验证函数被调用
        self.assertEqual(result, 0)
    
    @patch('main.cli.commands.handlers.handle_process_script')
    def test_process_script_command(self, mock_handle):
        """测试 process-script 命令"""
        mock_handle.return_value = 0
        
        # 创建测试文件
        input_file = self.test_path / 'input.tcl'
        output_file = self.test_path / 'output.tcl'
        input_file.write_text('puts "test"')
        ...
        from main.cli.commands.handlers import handle_process_script
        result = handle_process_script(manager, args)
        
        # 验证函数被调用
        self.assertEqual(result, 0)
    
    @patch('main.cli.commands.handlers.handle_load_workflow')
    def test_load_workflow_command(self, mock_handle):
        """测试 load-workflow 命令"""
        mock_handle.return_value = 0
        ...
        from main.cli.commands.handlers import handle_load_workflow
        result = handle_load_workflow(manager, args)
        
        # 验证函数被调用
        self.assertEqual(result, 0)
    
    @patch('main.cli.commands.handlers.handle_run_workflow')
    def test_run_workflow_command(self, mock_handle):
        """测试 run 命令（完整工作流）"""
        mock_handle.return_value = 0
        ...
        from main.cli.commands.handlers import handle_run_workflow
        result = handle_run_workflow(manager, args)
