...
def handle_run_range(manager, args, run_from, run_to, single_step) -> int:
    ...
    try:
        ...
        branch_dir = work_path / work_path_info['project'] / work_path_info.get('version') / \
                     work_path_info['block'] / work_path_info['user'] / work_path_info['branch']
        ...
        print(f"[ERROR] 执行步骤范围时发生错误: {e}", file=sys.stderr)
        import traceback
        traceback.print_exc()
        return 1
